...
window._APP.captarVagasIDT=async function(){
...
// FIM CAPTURA IDT
window._APP.uploadVagasCSV=async function(event){
...
window._APP.carregarVagas=async function(){
...
// FIM VAGAS ADMIN
window._APP.renderVagas=function(lista){
  const el=document.getElementById('lista-vagas');
...
window._APP.filtrarVagasCidade=function(cidade){
  const el=document.getElementById('lista-vagas');
  el.dataset.filtroCity=cidade;
  APP.renderVagas(vagasData);
};
window._APP.filtrarVagas=function(){
...
// MAPA SERVIÇOS
window._APP.filtrarMapaSrv=function(tipo){
  mapaServFiltro=tipo;
...
window._APP.renderMapaServicos=function(){
  if(!mapaServicos){
...
window._APP.editarEvento=function(id){
...
window._APP.evCancelarEdicao=function(){
...
window._APP.evBuscarLocal=async function(){
...
window._APP.uploadBannerEvento=async function(event){
  const file=event.target.files[0]; if(!file) return;
...
window._APP.salvarEvento=async function(){
...
window._APP.delEvento=async function(id){if(!confirm('Excluir?'))return;try{await deleteDoc(doc(db,'eventos',id));await carregarEventos();}catch(e){alert('Erro.');}};
...
window._APP.salvarCurso=async function(){
...
window._APP.delCurso=async function(id){if(!confirm('Excluir?'))return;try{await deleteDoc(doc(db,'cursos',id));await carregarCursos();}catch(e){alert('Erro.');}};
...
window._APP.abrirCadMot=function(){
...
window._APP.selecionarCurriculo=async function(event){
  const file=event.target.files[0]; if(!file) return;
...
window._APP.salvarMotorista=async function(){
...
window._APP.filtrarMotoristas=function(){
...
window._APP.delMotorista=async function(id){if(!confirm('Excluir?'))return;try{await deleteDoc(doc(db,'motoristas',id));await carregarMotoristas();}catch(e){alert('Erro.');}};

// ===== PRESTADORES - VERIFICAÇÃO E TERMOS =====
window._APP.verificarLoginPrestador=function(){
  // Função removida - agora abre a tela direto
};
window._APP.fecharTermosPrestador=function(){
  document.getElementById('modal-termos-prestador-overlay').style.display='none';
  window._cadastroPendente=null;
};
window._APP.salvarTermosPrestador=async function(){
...
window._APP.carregarNoticias=async function(tema){
...
window._APP.renderNoticias=function(lista){
  const el=document.getElementById('lista-noticias');
...
window._APP.filtrarNoticias=function(tema){
  temaAtual=tema;
...
window._APP.lojaFiltrar=function(cat, btn){
...
window._APP.lojaBuscar=function(){
...
window._APP.lojaRenderGrid=function(){
...
// ===== PROGRAMA DE RÁDIO =====
window._APP.mostrarAbaRadio=function(aba){
...
window._APP.enviarFormularioRadio=function(tipo){
  let dados={};
  if(tipo==='alo'){
    dados={
      tipo:'alo',
...
window._APP.abrirInscricao=function(evId){
...
window._APP.fecharInscricao=function(){
  document.getElementById('modal-inscricao').style.display='none';
  document.body.style.overflow='';
};
window._APP.enviarInscricao=function(){
...
// Sincroniza window.APP com window._APP para onclick inline no HTML
